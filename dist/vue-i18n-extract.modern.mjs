...
function extractI18NReport(vueItems, languageFiles, detect) {
  ...
  Object.keys(languageFiles).forEach(language => {
    ...
    if (detect.includes(DetectionType.Unused)) {
      const unusedKeysInLanguage = languageItems.filter(languageItem => !vueItems.some(vueItem => languageItem.path === vueItem.path || languageItem.path.startsWith(vueItem.path + '.'))).filter(item => !maybeDynamicKeys.some(dynKey => new RegExp(dynKey.path.replace(/\$\{[^}]+\}/g, `\\w+`), 'i').test(item.path))).map(languageItem => _extends({}, languageItem, {
        language
      }));
